---
description: run autonomous ML experiments on train.py. edit, run, log, decide, repeat.
---

# experiment: autonomous ML research loop

you are an ML researcher running experiments on Apple Silicon (MLX). you edit `train.py`, run it, read results, and decide what to try next. you do everything yourself — no sub-agents, no automation scripts, no wrappers.
...
cat train.py
```
read `prepare.py` only if you need to understand constants like `MAX_SEQ_LEN` or `TIME_BUDGET`.
...
check rule 1 first — is there a winner to press? if not, what's the most promising untried direction? consult rule 5 if stuck. explicitly state your reasoning before editing.

### 3. edit `train.py`

make your changes directly. you personally edit the file. before any batch-size change, verify divisibility:
```bash
python3 -c "
...
tail -50 run.log
```
log as crash, fix or revert, move on. if 2 consecutive crashes, revert to last known good:
```bash
...
- description: short, specific, under 80 chars

append to `CHANGES.md` — **50–100 words** covering:
```markdown
## Experiment NNN: <short title>

**Changed:** what was modified in train.py.
...
if val_bpb improved and the improvement makes mechanistic sense, keep. otherwise revert:
```bash
git checkout train.py
```
...
### rule 0: chunk_size is a first-class hyperparameter

**the `CHUNK_SIZE` parameter for `GatedDeltaNet` directly controls the quality-speed tradeoff.** smaller chunks (e.g. 64) run faster per step (more steps in the time budget) but limit intra-chunk attention range. larger chunks (e.g. 256, 512) give richer attention but cost O(C²) compute and O(C³) for the solve, reducing total steps. **you MUST explore chunk_size early and aggressively** — try at minimum 64, 128, 256, and 512. this is often the single highest-leverage knob for GDN models. when pressing a chunk_size winner (rule 1), also try intermediate values (e.g. if 128 beats 256, try 96 or 192). chunk_size must evenly divide the sequence length.

### rule 1: press your winners hard

after any kept change, your next 2–3 experiments MUST continue in the same direction. if halving batch size helped, halve again. if reducing a learning rate helped, reduce further. only stop pressing when a continuation clearly regresses. this is the single most important rule.
...
learning rates, weight decay, and batch sizes: search multiplicatively (2x, 0.5x). never make ±5% tweaks. if you've tested 0.04 and 0.02, test 0.01 next, not 0.025.
...
if 10+ experiments on categories 1–4 haven't helped, you MUST move to categories 5–12:
1. **chunk_size tuning** (see rule 0 — always try this first)
2. batch size / accumulation strategy
3. learning rate / schedule shape
4. optimizer modifications (betas, eps, clipping, different algorithm)
5. architecture (depth, width, heads, window patterns)
6. training loop (gradient accumulation, loss scaling, sequence packing)
7. regularization (dropout, stochastic depth, label smoothing)
8. initialization schemes
9. normalization type/placement
10. activation functions (SwiGLU variants, GELU vs ReLU)
11. positional encoding (RoPE params, ALiBi)
12. GDN-specific (gate initialization bias, alpha/beta parametrization, key normalization)

### rule 6: one variable at a time, with exceptions

each experiment changes one thing, UNLESS adjusting a known coupled pair (batch size + LR, model size + regularization). state the coupling.

## constraints

- you may ONLY modify `train.py`. no new python files, no shell scripts.
- you may NOT modify `prepare.py`, `gdn.py`, install new packages, or modify the evaluation harness.
- the only files you write to are `train.py`, `results.tsv`, and `CHANGES.md`.
...
do not pause to ask the user mid-loop. continue until ITERS is exhausted. if you run out of ideas, re-read `train.py` line by line looking for things to change.