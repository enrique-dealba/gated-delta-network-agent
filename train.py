...
class CausalSelfAttention(nn.Module):
    def __init__(self, config, layer_idx):
        ...
        self.gdn = GatedDeltaNet(config.n_embd, config.n_head, chunk_size=config.chunk_size)
...
class GPT(nn.Module):
    ...
    def init_weights(self):
        ...
        for block in self.blocks:
            ...
            # Init GDN gate weights
            block.attn.gdn.W_alpha.weight = mx.zeros_like(block.attn.gdn.W_alpha.weight).astype(
                mx.bfloat16
            )
            block.attn.gdn.W_beta.weight = mx.zeros_like(block.attn.gdn.W_beta.weight).astype(
                mx.bfloat16
            )
    ...
    def __call__(self, idx, targets=None, reduction="mean"):
        ...
        for i, block in enumerate(self.blocks):
            ...
            x = block(x, ve, None)  # mask no longer needed
        x = norm(x)
...
class AdamW:
    def __init__(
        self, model, unembedding_lr, embedding_lr, matrix_lr, weight_decay, adam_betas, scalar_lr
    ):
        self.param_config = {}
        self.adam_state = {}
